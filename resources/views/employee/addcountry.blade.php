    ...
    <div class="card">
      ...
      <div class="table-responsive width-95-per mx-auto">
        ...
        @foreach($user as $user)
          ...
          <td> <a class="dropdown-item" href="{{url('/admin/editcountry',$user->id)}}">
                    <i data-feather="edit-2" class="me-50"></i>
                  </a></td>
         <td> <button class="btn btn-flat btn-sm remove-user" data-id="{{ $user->id}}" data-action="{{ url('/admin/deletecountry/',$user->id) }}" onclick="deleteConfirmation({{$user->id}})">
  ...
  function deleteConfirmation(id) {
    swal.fire({
      ...
      if (e.value === true) {
        ...
        $.ajax({
          type: 'GET',
          url: "{{url('admin/deletecountry')}}/" + id,
          data: {
            _token: CSRF_TOKEN
          },
          dataType: 'JSON',
